...
### MCP Tools
- 230 Revit tools with `--toolsets all` (233 with adaptive bake). Default surface is **41** tools (`query,create,view,meta`). ALL MCP-facing names prefixed `revit_` (e.g. `revit_create_grid`). Server↔plugin wire command names stay unprefixed and unchanged.
