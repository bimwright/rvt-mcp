...
v0.6.2 には、ドア・窓の明示的なホスト指定と実際の位置の検証（#13）、配管延長時のシステム継承と異なるシステムタイプ間の接続拒否（#12）、MEP ネットワーク要素の集計修正（#11）が含まれます。[動作と使用例](docs/placement-and-mep-contracts.md)、[検証記録](docs/testing/2026-09-22-issue-handoff.md)を参照してください。サーバーとプラグインを同時に更新し、Revit と MCP 接続を再起動してください。
